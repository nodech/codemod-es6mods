...
flags.MANDATORY_VERIFY_FLAGS = 0
  | flags.VERIFY_MINIMALDATA
  | flags.VERIFY_MINIMALIF
  | flags.VERIFY_NULLFAIL;
...
flags.ONLY_STANDARD_VERIFY_FLAGS =
  flags.STANDARD_VERIFY_FLAGS & ~flags.MANDATORY_VERIFY_FLAGS;
...
export function isKeyEncoding(key) {
...
export function isSignatureEncoding(sig) {
  ...
  type &= ~hashType.NOINPUT;
  type &= ~hashType.ANYONECANPAY;

  if (type < hashType.ALL || type > hashType.SINGLEREVERSE)
    return false;
...
export function toASM(item, decode) {
  if (item.length <= 4) {
  ...
  if (decode && isSignatureEncoding(item)) {
    ...
    let symbol = hashTypeByVal[type & 0x1f] || '';
